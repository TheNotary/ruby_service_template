module RubyServiceTemplate
  ...
  class Installer
    ...
    def install(service_name = SERVICE_NAME, target_folder = 'c:\it\ipfixer')
      ...
      ruby_path = get_ruby_interpreter_path
      gem_path = get_ipfixer_bin_path
      service_command_line = construct_service_execution_string(ruby_path, gem_path)
    ...
    def prompt_for_information
      #  FIXME:  Make this easy to configure... or something...  Or put in a hello world type of input
    ...
    def update_yml_file(target_folder, config)
      # FIXME:  Come up with a way to dry this up so that it is specified in the prompt_for_information
      return if true
